"""SUPERSEDED -- the first attempt's profiling pass. Do not read this file as the submission.
...
The maintained implementation is::
    ...
    solution/src/profiling/ (profiler.py, checks.py)

The maintained profiler runs BEFORE any cleaning, so `profile_report.json` is evidence
captured while the defects were still present.
...
raise RuntimeError(
    "src/profiler.py at the repository root is a SUPERSEDED first attempt and is retained "
