"""SUPERSEDED -- analytics tests from the first attempt. Not the submitted suite.
...
The maintained suite splits these: `test_analytics.py` runs the metric SQL against a
synthetic warehouse, and `test_metric_contracts.py` treats the metric registry as an
interface (required ids, `column_units` completeness, window-boundary arithmetic, and a
falsifiable reconciliation delta that a known $5.00 injected error must reproduce exactly).
...
pytest.skip(
    "SUPERSEDED: tests/test_analytics.py at the repository root belongs to the first attempt. "
