      ...
      <div className="flex flex-wrap items-center justify-between gap-4 rounded-lg border border-line bg-panel p-4">
        <div>
          ...
          <p className="text-xs text-ink-dim">
            <span className="text-accent font-semibold">Select a red cell and ask assistant</span>, or inspect its seeded defect, reason code, and live <span className="text-green-400 font-semibold">15s flashing Green fix</span> in the clean pipeline output. Both panes hold {displayRows.length} rows; only the visible rows are in the DOM.
          </p>
        </div>
        ...
        <div className="space-y-3 rounded-lg border border-accent/40 bg-raised p-4 shadow-md">
          ...
          <div className="mt-3 flex flex-wrap items-center justify-end gap-2 pt-2 border-t border-line/50">
            {onOpenAssistant ? (
              ...
              >
                <span>Step 2:</span>
                <span>Select a red cell and ask assistant &rarr;</span>
              </button>
            ) : (
              <a
                href="#assistant"
            ...
            {activeCell.info.defect_code && onSelectDefect && (
              ...
              >
                <span>Step 3:</span>
                <span>View {activeCell.info.defect_code} in Defect Explorer &amp; Linked Code &rarr;</span>
              </button>
            )}
          </div>
        </div>
          ...
          <DiffPane
            ...
            legend={
              <span className="inline-flex items-center gap-1.5 rounded-full border border-red-500/40 bg-red-500/15 px-2.5 py-0.5 text-2xs text-red-300 font-semibold shadow-sm">
                ...
                <span>Select a red cell and ask assistant</span>
              </span>
            }
          />
        )}
